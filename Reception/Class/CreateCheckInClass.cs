...
{
    public class CreateCheckInClass
    {

        ReseptionEntities db = new ReseptionEntities();

        public void CreateDoc(int idworker, int idCheck, RoomForCheckIn roomFor, List<ServiceFull> sf)
        {
            try
            {
                List<CheckIn> checkIns = db.CheckIn.Where(x => x.ID == idCheck).ToList();
                List<Worker> workers = db.Worker.Where(x=>x.ID == idworker).ToList();
                List<Room> rooms = db.Room.Where(x => x.ID == roomFor.ID).ToList();
                var days = 0.0;
                var day = 0.0;
                ...
                foreach (var i in rooms)
                    para1.Range.Text = "Проживание: " + $"{days} дн Х {Math.Round(i.Cost)}р";
                para1.Range.InsertParagraphAfter();
                ...
                foreach(var i in checkIns)
                    para2.Range.Text = $"Итого: {Math.Round(i.Sum)}р";
                ...
                //Save the document
                object filename = @"D:\Курсовая 4 курс с Глебом\checkReception.docx";
